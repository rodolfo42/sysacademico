...
public class MatriculaController {
	...
	@Get @Path("/matricula/adicionar")
	public void adicionar() {
		incluirListasNaResult();
	}
	
	@Post @Path("/matricula/adicionar")
	public void adicionar(Matricula matricula) {
		dao.salva(matricula);
		result.redirectTo(this).listar();
	}
	
	@Get @Path("/matricula/{id}")
	public Matricula editar(Long id) {
		incluirListasNaResult();
		return dao.carrega(id);
	}
	
	@Put @Path("/matricula/{matricula.id}")
	public void alterar(Matricula matricula) {
		dao.atualiza(matricula);
		result.redirectTo(this).listar();
	}
	
	@Delete @Path("/matricula/{id}")
	public void deletar(Long id) {
		dao.deleta(id);
		result.redirectTo(this).listar();
	}

	@Get @Path("/matricula/listar")
	public List<Matricula> listar() {
		return dao.listaTudo();
	}
	
	@Get @Path("/matricula/busca.json")
	public void buscaJson(String nomeAluno) {
		if (!UtString.isNullOrEmpty(nomeAluno)){
			dao.buscarPorAluno(nomeAluno);
		}

		result.use(json()).from(dao.buscar()).include("aluno")
			.include("responsavel").include("curso").include("listaTipoAula","listaTipoAula.nome").serialize();
	}
	
	@Get @Path("/matricula/aluno.json")
	...
	@Get @Path("/matricula/responsavel.json")
