...
public class MatriculaController {
	...
	public MatriculaController(MatriculaDao matriculaDao, AlunoDao alunoDao,
			ResponsavelDao responsavelDao, CursoDao cursoDao, Result result) {
	...
	public void buscaJson(String nomeAluno) {
		...
		result.use(json()).from(dao.buscar()).include("aluno")
				.include("responsavel").include("curso")
	...
	public void alunoJson(String idResponsavel) {
		if (!idResponsavel.equals("0")) {
			...
			result.use(json()).from(alunoDao.listaPeloResponsavel(responsavel))
					.serialize();
		} else {
	...
	public void responsavelJson(String idAluno) {
		if (!idAluno.equals("0")) {
			...
			result.use(json()).from(responsavelDao.listaPeloAluno(aluno))
					.serialize();
		} else {
