...
<table>
	<tr>
		<th align="left"><label for="${idComboSeleciona }">${labelSeleciona }:</label>
		</th>
		<th>&nbsp;</th>
		<th align="left"><label for="${idComboSelecionado }">${labelSelecionado }:</label>
		</th>
	</tr>
	<tr>
		<td><select name="listaSelecionado" id="${idComboSeleciona }" multiple="multiple">
				<c:forEach items="${listaSeleciona }" var="seleciona">
					<option value="${seleciona.id }">${seleciona.nome }</option>
				</c:forEach>
		</select></td>
		<td valign="middle"><i class="icon-fast-backward seta-esquerda"></i> <i class="icon-fast-forward seta-direita"></i>
		</td>
		<td><select name="${nameSelecionado }" id="${idComboSelecionado }" multiple="multiple">
